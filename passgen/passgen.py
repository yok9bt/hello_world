...
def do_iterations(data = 'Give me data!'.encode(), number = 0, verbose = True):
    for i in range(number):
        for _ in range(int(1e6)):
            data = hashlib.sha3_512(data).digest()
        if verbose: print(data.hex(), ' (iteracja: ', i+1,') (pozostało: ', number-(i+1), ')', sep='')

def speed_test(test_loops = 1):   # output is in MH/h
    start = time.time()
    do_iterations('speed_test'.encode(), test_loops, False)
    return int((test_loops / (time.time() - start))*3600)

def main():
    string1 = input('String wejściowy: ').encode()
